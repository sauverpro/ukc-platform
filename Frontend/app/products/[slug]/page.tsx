...
export default function ProductDetailPage() {
  ...
  if (!product) return <div className="p-20 text-center" style={{ color: textPrimary }}>Product not found.</div>;
    ...
    <div className="max-w-5xl mx-auto" style={{ background: bg, fontFamily: "var(--font-inter, system-ui, sans-serif)" }}>
      ...
      <main className="mx-auto px-8 md:px-16" style={{ maxWidth: "900px", paddingTop: "100px", paddingBottom: "80px" }}>
        ...
        <p className="text-xs mb-8" style={{ color: textMuted }}>
          ...
          <Link href="/products" className="hover:text-[#0d9e72] transition uppercase">Uncategorized</Link>
          <span className="mx-2">›</span>
        ...
        <div className="grid md:grid-cols-2 gap-12 mb-16">
          ...
          <div>
            ...
            {product.rating > 0 && <div className="mb-3"><Stars count={product.rating} reviews={1} /></div>}
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <Button
                className="flex-1"
                onClick={() => { if (inCart) router.push("/cart"); else addItem({ title: product.title, img: product.img, price: product.priceNum }); }}
              >
            ...
            <p className="text-xs" style={{ color: textMuted }}>
              ...
              <span style={{ color: "#0d9e72" }}>UNCATEGORIZED</span>
            </p>
          </div>
        </div>
        ...
        <div style={{ borderBottom: `1px solid ${border}` }} className="flex gap-8 mb-6">
          {(["description", "reviews"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "reviews" ? "Reviews (1)" : "Description"}
            </button>
          ))}
        </div>
        ...
        <div className="mb-16 text-sm" style={{ color: textMuted }}>
          {tab === "description" ? (
            <p>
              <span style={{ color: "#0d9e72" }}>Total</span>{" "}
              <span style={{ color: "#0d9e72" }}>Mixed</span>{" "}
              <span style={{ color: "#0d9e72" }}>Ration</span>{" "}(TMR)
            </p>
          ) : (
            ...
            return (
              <div key={p.slug} className="group flex flex-col" style={{ background: cardBg }}>
                ...
                <div className="pt-3 pb-4 flex flex-col items-center gap-1 px-2">
                  ...
                  <p className="text-xs font-semibold tracking-widest" style={{ color: textMuted }}>UNCATEGORIZED</p>
                  <Button
                    size="sm"
                    onClick={() => { if (relAdded) router.push("/cart"); else addItem({ title: p.title, img: p.img, price: p.priceNum }); }}
                  >
