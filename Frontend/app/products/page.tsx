...
function ProductCard({ slug, title, img, price, priceNum, rating, index }: any) {
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    let sortQuery = "";
    if (sort === "price-asc") sortQuery = "&sort=price_asc";
    if (sort === "price-desc") sortQuery = "&sort=price_desc";
    if (sort === "latest") sortQuery = "&sort=latest";

    api.get<PaginatedResponse<Product>>(`/products?per_page=12${sortQuery}`)
      .then((res) => {
  ...
  const sortedFallback = [...FALLBACK_PRODUCTS].sort((a, b) => {
    if (sort === "popularity" || sort === "rating") return b.rating - a.rating;
    if (sort === "latest") return FALLBACK_PRODUCTS.indexOf(b) - FALLBACK_PRODUCTS.indexOf(a);
    if (sort === "price-asc") return a.priceNum - b.priceNum;
    if (sort === "price-desc") return b.priceNum - a.priceNum;
    return 0;
  });

  const displayProducts = products === FALLBACK_PRODUCTS && sort !== 'default' ? sortedFallback : products;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="page-hero" style={{ minHeight: "220px" }}>
        <div className="page-hero__bg" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1530836369250-ef72a3f5cda8?w=1600&q=80')" }} />
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-10">
            <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "#9ca3af" }}>
              Showing all {displayProducts.length} results
            </p>
